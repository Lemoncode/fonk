...
const setValues = (newValues, index) => {
  values = { ...newValues };
  const nameElement: any = document.getElementById(`users[${index}].name`);
  nameElement.value = values.users[index].name;
  const emailElement: any = document.getElementById(`users[${index}].email`);
  emailElement.value = values.users[index].email;
  const repeatEmailElement: any = document.getElementById(
      ...
      .then((validationResult) => {
        setErrors({
          ...
          [`users[${index}].${fieldName}`]:
            validationResult[`users[${index}].${fieldName}`],
        });
      });
  });
};
